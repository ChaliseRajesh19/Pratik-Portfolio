...
function UploadForm({ categories, defaultCategory, onUploaded, initialWork, onCancel }) {
  ...
  const handleSubmit = async (event) => {
    ...
    if (!initialWork && !previewImage) { toast.error("Please select at least one main image."); return; }

    try {
      setLoading(true);
      if (initialWork) {
        await updateWork(initialWork._id, {
  ...
  return (
    <div className="h-full w-full overflow-y-auto custom-scrollbar px-6 py-6 md:px-8">
      ...
      <p className="mt-2 text-sm text-slate-400">Add images to a category and optionally attach one short headline.</p>
      ...
      <form onSubmit={handleSubmit} className="mt-6 space-y-5 max-w-6xl">
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          <div className="flex flex-col">
            <span className="block text-sm font-medium text-slate-300 mb-2">Main Image</span>
            <label className="relative rounded-2xl border border-dashed border-slate-700/80 bg-slate-950/40 flex flex-col items-center justify-center text-center overflow-hidden group min-h-[180px] cursor-pointer hover:border-purple-500/50 transition-colors">
              <input type="file" accept="image/*" onChange={(e) => {
                ...
                if (f) { setPreviewImage(f); setPreviewImageUrl(URL.createObjectURL(f)); }
              }} className="absolute inset-0 w-full h-full opacity-0 cursor-pointer z-10" />
              {previewImageUrl ? (
                <img src={previewImageUrl} className="absolute inset-0 w-full h-full object-cover opacity-55 group-hover:opacity-75 transition-opacity" alt="Preview" />
              ) : null}
              ...
              <div className="text-sm font-semibold text-slate-200 relative z-20">
                {previewImage ? previewImage.name : previewImageUrl ? "Change Main Image" : "Upload Main Image"}
              </div>
            </label>
            <p className="mt-2 text-[11px] text-slate-500 italic">This image is used in the public category grid first.</p>
          </div>
          ...
          <div className="flex flex-col">
            ...
            <div className="relative rounded-2xl border border-dashed border-slate-700/80 bg-slate-950/40 p-6 flex flex-col items-center justify-center text-center overflow-hidden group min-h-[180px] hover:border-purple-500/50 transition-colors">
              <input type="file" accept="image/*" multiple onChange={(e) => setGalleryImages(prev => [...prev, ...Array.from(e.target.files || [])])}
                className="absolute inset-0 w-full h-full opacity-0 cursor-pointer z-10" />
              <div className="flex flex-col items-center pointer-events-none relative z-20">
                ...
                <div className="text-sm font-semibold text-slate-200">
                  {galleryImages.length + existingGalleryImages.length > 0 ? `${galleryImages.length + existingGalleryImages.length} additional images` : "Upload More Images"}
                </div>
              </div>
              {galleryImages.length > 0 && (
                <button type="button" onClick={(e) => { e.stopPropagation(); setGalleryImages([]); }}
            ...
            <p className="mt-2 text-[11px] text-slate-500 italic">These appear in the lightbox/gallery for users.</p>
          </div>
        </div>
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-3">
              {existingGalleryImages.map((imgUrl, idx) => (
                <div key={`exist-${idx}`} className="relative w-24 h-24 rounded-xl overflow-hidden border border-slate-700 group shadow-lg">
                  <img src={imgUrl} className="w-full h-full object-cover bg-slate-800" alt="Existing" />
                  <button type="button" onClick={() => setExistingGalleryImages(prev => prev.filter((_, i) => i !== idx))}
              ...
              {galleryImages.map((file, idx) => {
                ...
                return (
                  <div key={`new-${idx}`} className="relative w-24 h-24 rounded-xl overflow-hidden border border-purple-500/50 group shadow-lg">
                    <img src={objUrl} className="w-full h-full object-cover" alt="New" />
                    <span className="absolute top-0 right-0 bg-purple-500 text-white text-[9px] font-bold px-1 rounded-bl leading-tight">NEW</span>
                    <button type="button" onClick={() => { URL.revokeObjectURL(objUrl); setGalleryImages(prev => prev.filter((_, i) => i !== idx)); }}
                      className="absolute inset-0 bg-black/70 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
                      <svg className="w-5 h-5 text-rose-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2"><path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12" /></svg>
                    </button>
                  </div>
