  ...
  return (
    <div className="h-full w-full overflow-y-auto px-6 py-6 md:px-8">
      <div className="mb-6 max-w-5xl">
        ...
        <p className="mt-2 text-sm text-slate-400">Set up the category name, slug, icon, description, and display order in a full-width workspace.</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-5 max-w-5xl">
          ...
          <input
            ...
            onChange={(event) => {
              ...
              if (!slug || slug === name.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)+/g, '')) {
                setSlug(nextName.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)+/g, ''));
              }
            }}
            required
        ...
        <label className="block text-sm font-medium text-slate-300">
          ...
          <textarea
            ...
            required
            rows={3}
            placeholder="This text appears at the top of the category page..."
