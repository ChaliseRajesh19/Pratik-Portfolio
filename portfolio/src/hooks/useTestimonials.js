...
function formatTestimonial(row) {
  ...
  return {
    ...
    id: row.id,
    name: row.name,
    role: row.role || row.company || '',
    company: row.company || row.role || '',
...
export function useTestimonials() {
  ...
  const createTestimonial = useCallback(async ({ name, role, company, rating, text, quote, avatarFile, avatarUrl: existingAvatar }) => {
    ...
    const { data, error } = await supabase
      .from('testimonials')
      .insert([{ name, role: role || company, company: company || role, avatar_url: avatarUrl, rating: rating || 5, text: text || quote, quote: quote || text }])
      .select()
      .single();
  ...
  const updateTestimonial = useCallback(async (id, { name, role, company, rating, text, quote, avatarFile, avatarUrl: existingAvatar }) => {
    if (!supabase) throw new Error('Supabase client is not configured');
    const updateData = { name, role: role || company, company: company || role, rating, text: text || quote, quote: quote || text, updated_at: new Date().toISOString() };
    if (avatarFile) {
      updateData.avatar_url = await uploadFile(avatarFile, BUCKET, 'testimonials');
    } else if (existingAvatar) {
      updateData.avatar_url = existingAvatar;
    }
    const { data, error } = await supabase
      .from('testimonials')
      .update(updateData)
