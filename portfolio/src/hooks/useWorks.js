...
function formatWork(row) {
  ...
  return {
    ...
    galleryImages: row.gallery_images || row.gallery || [],
    displayOrder: Number.isFinite(Number(row.display_order || row.order || row.index)) ? Number(row.display_order || row.order || row.index) : 0,
