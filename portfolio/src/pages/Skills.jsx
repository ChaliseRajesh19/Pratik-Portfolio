...
function Skills({ withTopOffset = true }) {
  return (
    <section
      className={`min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 text-slate-100 ${withTopOffset ? 'mt-16' : 'mt-0'}`}
    >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12">
          {skillCategories.map((category, categoryIndex) => (
            ...
            >
              {/* Category Header */}
              <div className="mb-6">
                <h3 className="text-2xl font-bold text-cyan-400 mb-2">{category.category}</h3>
                ...
                {category.skills.map((skill, skillIndex) => (
                  ...
                  >
                    ...
                    <div className="relative w-full bg-slate-700/50 rounded-full h-3 overflow-hidden">
                      {/* The bar animates width from 0 → level when scrolled into view */}
                      <motion.div
                        initial={{ width: 0 }}
                        ...
                        className="absolute top-0 left-0 h-full bg-gradient-to-r from-cyan-500 via-blue-500 to-cyan-400 rounded-full group-hover:shadow-lg group-hover:shadow-cyan-500/50 transition-shadow duration-300"
                      >
