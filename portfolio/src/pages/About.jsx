...
/* ── Orbit badge positions ───────────────────────────────── */
const orbitBadges = [
  { label: "Brand Systems",    top:    "7%",  left:  "4%",  delay: 0   },
  { label: "Motion Cuts",      top:    "16%", right: "2%",  delay: 0.7 },
  { label: "Visual Identity",  bottom: "17%", left:  "0%",  delay: 1.4 },
  { label: "Campaign Design",  bottom: "10%", right: "4%",  delay: 2.1 },
];
...
function ScrollRevealTextContainer({ paragraphs }) {
  ...
  return (
    <div ref={containerRef} className="space-y-6 mt-6">
      {paragraphs.map((text, pi) => {
        ...
        return (
          <p
            key={pi}
            className="flex flex-wrap gap-x-[0.3em] gap-y-[0.2em] text-slate-300 text-base leading-relaxed"
          >
            {words.map((word, i) => {
...
function About({ withTopOffset = true }) {
  ...
  /* Scroll-driven parallax for the image stage only */
  const { scrollYProgress } = useScroll({
    target: stageRef,
  ...
  const ringRotate  = useTransform(scrollYProgress, [0, 1], [-16, 20]);
  const cRingRotate = useTransform(ringRotate, v => v * -1.3);
  const imageScale  = useTransform(scrollYProgress, [0, 0.5, 1], [0.96, 1.04, 0.98]);

  const sectionClass = `min-h-screen px-4 py-10 md:py-24 ${withTopOffset ? "mt-16" : "mt-0"}`;

  return (
    <SectionMotionShell variant="blue" ghostLabel="about" className={sectionClass}>
      <div className="max-w-6xl mx-auto">
        <div className="grid md:grid-cols-2 gap-12 lg:gap-16 items-center">
          ...
          <div>
            ...
            {/* Scroll-bound per-word reveal with blur+fade cascade */}
          ...
          <div ref={stageRef} className="flex items-center justify-center h-full">
            ...
            >
              ...
              <motion.div
                ...
                style={{ rotate: cRingRotate }}
              />

              {/* Inner ring — pure CSS auto-rotate, isolated from scroll */}
              <motion.div
                aria-hidden="true"
              ...
              <motion.div
                aria-hidden="true"
                className="absolute inset-[12%] rounded-[38%] bg-[radial-gradient(circle,rgba(59,130,246,0.26),transparent_62%)] blur-3xl"
                style={{ scale: imageScale }}
              />

              {/* Orbit badges — Decoupled animations to prevent stuttering/bugs */}
              {orbitBadges.map(({ label, delay, ...position }) => (
                ...
                >
                  <motion.div
                    className="rounded-full border border-blue-400/22 bg-slate-950/70 px-4 py-2 text-[10px] font-bold uppercase tracking-[0.2em] text-blue-200 shadow-[0_12px_28px_rgba(2,6,23,0.45)] backdrop-blur-xl whitespace-nowrap"
                    animate={{
                      y: [0, -8, 0],
                      opacity: [0.85, 1, 0.85],
                    }}
              ...
              <motion.div
                ...
                className="relative z-10 w-[78%] max-w-md cursor-default"
              >
                <div className="absolute -inset-6 rounded-[2rem] bg-[radial-gradient(circle,rgba(59,130,246,0.20),transparent_65%)] blur-2xl opacity-70" />
                <div className="absolute inset-0 rounded-[2rem] border border-white/10 bg-gradient-to-br from-white/10 to-transparent" />
                <motion.div
                  aria-hidden="true"
                  className="absolute inset-0 rounded-[2rem] bg-gradient-to-br from-blue-400/18 via-transparent to-cyan-300/12"
                ...
                <motion.img
                  src={ProfileImage}
                  alt="Pratik Bhusal"
                  className="relative w-full h-auto rounded-[2rem] object-cover border border-white/[0.08]"
                  style={{
                    scale: imageScale,
